...
export default function ProductionBoard({ tickets = [], isFullscreen = false }) {
    ...
    // Detect changes and highlight updated tickets
    useEffect(() => {
        ...
        tickets.forEach(ticket => {
            ...
            if (prev) {
                // Check if ticket moved to a different step or quantity changed
    ...
    // Group tickets by their current workflow step
    const groupTicketsByWorkflow = () => {
        ...
        tickets.forEach(ticket => {
            if (ticket.status === 'completed') {
                ...
                // Find the first active workflow step for this job type
    ...
    // Show all workflow steps (not just active ones)
                ...
                <div className={`card-body ${isFullscreen ? 'p-3' : 'p-2'}`}>
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-1">
                        <small className="text-muted" style={{ fontSize: isFullscreen ? '0.85rem' : '0.75rem' }}>
                            <i className="ti-package"></i> {ticket.produced_quantity || 0}/{ticket.total_quantity || ticket.quantity}
                        </small>
                        <small className="text-muted" style={{ fontSize: isFullscreen ? '0.85rem' : '0.75rem' }}>
