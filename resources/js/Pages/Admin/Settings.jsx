...
export default function Settings({ settings: initialSettings, branches: initialBranches }) {
    const [settings, setSettings] = useState({
        payment_gcash_account_name:
            initialSettings?.payment_gcash_account_name || "",
        payment_gcash_number: initialSettings?.payment_gcash_number || "",
        payment_gcash_show_on_customer_page:
            initialSettings?.payment_gcash_show_on_customer_page !== undefined
    ...
    const [qrcodePreview, setQrcodePreview] = useState(
        initialSettings?.payment_gcash_qrcode || ""
    );
    const [customerOrderQrcodeFile, setCustomerOrderQrcodeFile] =
        useState(null);
    const [customerOrderQrcodePreview, setCustomerOrderQrcodePreview] =
        useState(initialSettings?.customer_order_qrcode || "");
    const [showUpload, setShowUpload] = useState(
        !initialSettings?.customer_order_qrcode
    );
    ...
    const handleCustomerOrderQRCodeUpload = (e) => {
        ...
        if (file) {
            ...
            const reader = new FileReader();
            reader.onloadend = () =>
                setCustomerOrderQrcodePreview(reader.result);
            reader.readAsDataURL(file);
        }
    };
    ...
    const removeBankAccount = (index) => {
        setSettings((prev) => ({
            ...prev,
            payment_bank_accounts: prev.payment_bank_accounts.filter(
                (_, i) => i !== index
            ),
        }));
    ...
    const handleSubmit = (e) => {
        ...
        formData.append(
            "payment_gcash_account_name",
            settings.payment_gcash_account_name
        );
        formData.append("payment_gcash_number", settings.payment_gcash_number);
        formData.append(
            "payment_gcash_show_on_customer_page",
            settings.payment_gcash_show_on_customer_page ? "1" : "0"
        );
        if (qrcodeFile) {
        ...
        formData.append(
            "payment_bank_accounts",
            JSON.stringify(settings.payment_bank_accounts)
        );

        Object.keys(bankQrcodeFiles).forEach((index) => {
            if (bankQrcodeFiles[index]) {
                formData.append(
                    `payment_bank_qrcode_${index}`,
                    bankQrcodeFiles[index]
                );
            }
        });
                        ...
                        <div className="card">
                            <div className="card-title">
                                ...
                                <p className="text-muted">
                                    Manage your business information and payment
                                    details
                                </p>
                            </div>
                            <div className="card-body">
                                <form onSubmit={handleSubmit}>
                                            ...
                                            <div className="col-md-12 mb-3">
                                                <div className="form-check">
                                                    <input
                                                        ...
                                                        type="checkbox"
                                                        checked={
                                                            settings.payment_gcash_show_on_customer_page
                                                        }
                                                        onChange={(e) =>
                                                            setSettings({
                                                                ...settings,
                                                    ...
                                                    <label
                                                        ...
                                                        htmlFor="showGcashOnCustomerPage"
                                                    >
                                                        Show GCash payment
                                                        details on customer
                                                        order page
                                    ...
                                    <div className="mb-4">
                                        ...
                                        {settings.payment_bank_accounts
                                            .length === 0 && (
                                                <div className="alert alert-info">
                                                    No bank accounts added yet.
                                                    Click "Add Bank Account" to add
                                                    one.
                                                </div>
                                            )}
                                        {settings.payment_bank_accounts.map(
                                            (account, index) => (
                                                <div
                                                    key={index}
                                                    className="card mb-3"
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <p className="text-muted small mb-3">
                                            Upload a QR code that customers can
                                            scan to be redirected to your online
                                            orders page. This QR code can be
                                            printed and displayed in your
                                            physical shop.
                                        </p>
                                        <div className="row">
                                            <div className="col-md-12 mb-3">
                                                <div className="d-flex justify-content-between align-items-center mb-2">
                                                    ...
                                                    {customerOrderQrcodePreview && (
                                                        <button
                                                            type="button"
                                                            className={`btn btn-sm ${showUpload
                                                ...
                                                {showUpload && (
                                                    <div className="mb-3 animate__animated animate__fadeIn">
                                                        <input
                                                            ...
                                                            accept="image/*"
                                                            onChange={
                                                                handleCustomerOrderQRCodeUpload
                                                            }
                                                        />
