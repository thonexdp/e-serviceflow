...
export default function CustomerPOSOrder() {
    ...
    const handleImageUpload = async (event) => {
        ...
        const totalFiles = designFiles.length + files.length;
        if (totalFiles > 3) {
            const remaining = 3 - designFiles.length;
            setDesignFileError(`You can only upload a maximum of 3 design files.`);
            event.target.value = "";
            return;
        }
    ...
    const handlePaymentProofUpload = async (event) => {
        ...
        // Check if adding these files would exceed the limit of 3
        const totalFiles = paymentProofs.length + files.length;
        if (totalFiles > 3) {
            const remaining = 3 - paymentProofs.length;
            setPaymentProofError(`You can only upload a maximum of 3 payment proof files. You currently have ${paymentProofs.length} file(s). You can add ${remaining} more.`);
            event.target.value = "";
            return;
        }
                                ...
                                paymentMethod === "bank") && (
                                    <div>
                                        ...
                                        <input
                                            ...
                                            disabled={paymentProofs.length >= 3}
                                        ...
                                        {paymentProofs.length >= 3 ? (
                                            <p className="text-xs text-orange-600 mt-1 font-medium">
                                                Maximum file limit reached. Remove a file to upload another.
                                            </p>
                                        ) : (
                                            <p className="text-xs text-gray-500 mt-1">
                                                Maximum 3 files • JPG, JPEG, PNG • Max 10MB per file
                                            </p>
                                        )}
                                                        ...
                                                        {paymentProofs.map(
                                                            (_, index) => (
                                                                <button
                                                                    ...
                                                                    className={`px-3 py-1 rounded text-sm whitespace-nowrap ${activeProofTab ===
                                                                        index
                                                                        ? "bg-orange-600 text-white"
                                                                        : "bg-gray-200 text-gray-700"
                                                                        }`}
